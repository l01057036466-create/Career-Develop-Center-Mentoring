...
// ==============================================================
const characterData = [
  { id: 1, name: "카마도 탄지로", type: "태양처럼 따뜻한 마음을 지닌 노력가", image: "https://i.namu.wiki/i/XDxdrDb5EfET9eod7H1StURXE21RMy-DRRAVrvnB14-qsZeKe7HEOhl9XQInz9FzAexvWEQr9T2ZduP6wU6O-y4khafcN1JUvM5_nyDIN_x8rl8kxdkaF_TDF6zwo9LzSxuPZmGDqAeYMSQ3YAe8yQ.webp", scores: [5, 5, 5, 2, 5, 5, 5, 2, 1, 1, 5, 1, 1, 5, 4, 1, 5, 4, 2, 5, 1, 1, 5, 1, 5, 2, 5, 2, 5, 5], 
    texts: { 
        analysis: { 
    ...
    // ... 나머지 24명의 캐릭터 데이터와 텍스트를 모두 여기에 붙여넣어야 합니다. 글자 수 제한으로 인해 생략합니다.
    // 이전 답변에서 제공한 characterData 전체를 여기에 복사해주세요.
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateSimilarity(userScores, characterProfile) {
      if (userScores.length !== characterProfile.scores.length) {
        console.error("Score arrays length mismatch!");
        return Infinity;
      }
    ...
    function calculateAndShowResult() {
        if (!characterData || characterData.length < 25) {
        ...
        const analysisHTML = Object.values(texts.analysis).map(part => `
            <div class="text-left space-y-2 p-4 border-b border-gray-700 last:border-b-0">
                <h4 class="text-xl font-semibold text-pink-300">${part.title}</h4>
                <p class="text-gray-300 leading-relaxed">${part.desc.replace(/\*\*(.*?)\*\*/g, '<strong class="text-pink-200">$1</strong>')}</p>
            </div>
        `).join('');
        ...
        resultScreen.innerHTML = `
            <div class="w-full text-center fade-in space-y-10">
                <!-- 1. 결과 발표 -->
                <div class="space-y-4">
                    <h1 class="text-3xl md:text-5xl font-bold text-white tracking-wider" style="text-shadow: 0 0 10px #f472b6;">당신의 영혼과 가장 닮은 검사는...</h1>
                    <div class="flex flex-col md:flex-row items-center justify-center gap-8 p-6 rounded-lg result-card">
                        <img src="${mainResult.image}" alt="${mainResult.name}" class="w-48 h-48 rounded-full border-4 border-pink-400 object-cover shadow-lg">
                        <div class="text-left space-y-2">
                            <h2 class="text-4xl font-bold text-pink-300">${mainResult.name}</h2>
                            <p class="text-xl text-gray-300">${mainResult.type}</p>
                            <p class="text-lg text-gray-300">성격 일치율: <span class="font-bold text-pink-300">${similarityPercentage}%</span></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 rounded-lg result-card space-y-4">
                    <h3 class="text-2xl font-bold border-b-2 border-pink-400 pb-2 mb-4">페르소나 프리즘 심층 분석</h3>
                ...
                <!-- 3. 관계 케미 분석 -->
                <div class="p-6 rounded-lg result-card space-y-4">
                     <h3 class="text-2xl font-bold border-b-2 border-pink-400 pb-2 mb-4">관계 케미 분석</h3>
                ...
                <div class="p-6 rounded-lg result-card space-y-4 text-left">
                     <h3 class="text-2xl font-bold border-b-2 border-pink-400 pb-2">멘토링 잠재력 분석</h3>
                     <h4 class="text-xl font-semibold text-pink-300">${texts.mentoring.role}</h4>
                     <p class="text-gray-300 leading-relaxed">${texts.mentoring.desc.replace(/\*\*(.*?)\*\*/g, '<strong class="text-pink-200">$1</strong>')}</p>
                </div>
            </div>
        `;
        showScreen('result-screen');
